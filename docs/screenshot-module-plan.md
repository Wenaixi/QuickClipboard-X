...
Windows 也提供离线原生 OCR：`Windows.Media.Ocr::OcrEngine` 可从用户已安装的语言包创建引擎，并对 `SoftwareBitmap` 调用 `RecognizeAsync`。它适合低延迟、无网络的纯文字转写，但受系统语言包和版面理解能力限制。第一期将其明确保留为未来“离线 OCR 模式”的候选，不作为 AI 识别的静默回退，避免用户以为图片没有离开本机而实际请求了云端，或以为 AI 结果具备原生 OCR 的离线保证。

项目已有 `aiApiKey`、`aiBaseUrl`、`aiModel` 配置。截图 AI 识别复用这些共享配置；不能假定现有纯文本翻译模型具备图片输入能力。
