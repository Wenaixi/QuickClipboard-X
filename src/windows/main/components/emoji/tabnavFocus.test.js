...
// 删除整个 tabbar 键盘死码链(enter-tabbar 意图无处产生)后,handleKbNav 系列
// 已整体移除,以下护栏断言改为"整链不存在"的否定形式。
...
// handleEmojiTabbarEnter / TabNavigation.kbEnter 死代码
...
test('TabNavigation 不暴露 kbEnter 给外部', async () => {
  const body = await readSource('../TabNavigation.jsx');
...
test('App.jsx 不再有 handleEmojiTabbarEnter', async () => {
...
// collapsedVisibleFilterCount 最小 4 后(commit 6af73f0f 产品决策:
...
// handleGroupRevealMouseMove 提前 return 分支(按钮已滑出/面板打开时)
