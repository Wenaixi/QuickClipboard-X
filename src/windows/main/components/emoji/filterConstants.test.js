...
// EmojiTab prev-mode 硬编码 ['emoji','symbols','images'],与 App 过滤路径/TabNavigation 常量脱钩。
// 修复:import cycleValue + EMOJI_MODE_IDS,cycleValue(EMOJI_MODE_IDS, ...) 复用单一真源。
test('EmojiTab prev-mode 复用 EMOJI_MODE_IDS 与 cycleValue,不再硬编码子模式数组', async () => {
...
// applyNavIntent 闭包读 emojiMode,deps 缺 emojiMode → stale 子模式决策。
test('applyNavIntent useCallback deps 必须含 emojiMode', async () => {
  const body = await readSource('../EmojiTab.jsx');
