...
// ImageLibraryTab 键盘边界口径不一致。activateKb 用 displayImageTotal
...
test('activateKb 与 kbMove 用同一已加载数组口径', async () => {
...
test('searchQuery 过滤时重置/夹取 kbImageIndex(过滤计算处)', async () => {
  const body = await readSource('./ImageLibraryTab.jsx');
...
// imageRowCount(Virtuoso totalCount 给视觉布局)用 displayImageTotal 全量,
...
test('imageRowCount 用 displayImageTotal(Virtuoso),与 kbMove 双口径是有意设计', async () => {
  const body = await readSource('./ImageLibraryTab.jsx');
