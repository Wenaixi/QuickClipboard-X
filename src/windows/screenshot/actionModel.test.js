...
test('actionForHotkey 未知键返回 null', () => {
  assert.equal(actionForHotkey('6'), null);
...
test('数字动作映射单一来源且正反向往返对称', () => {
  ...
  for (const action of ['copy', 'save', 'pin', 'ai', 'copy+pin']) {
...
test('动作映射全集封闭且每个动作恰好一个热键与防御式容错', () => {
  // 源码护栏：映射块必须恰好 5 个条目且与预期完全一致（加重复动作热键如 5: copy
  ...
  assert.deepEqual(entries, [['1', 'copy'], ['2', 'save'], ['3', 'pin'], ['4', 'ai'], ['5', 'copy+pin']], '映射条目必须与预期完全一致');
