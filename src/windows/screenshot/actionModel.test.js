...
test('actionForHotkey 数字键映射到对应动作', () => {
  ...
  assert.equal(actionForHotkey('5'), 'copy+pin');
  assert.equal(actionForHotkey('6'), 'ai');
});

test('actionForHotkey 未知键返回 null', () => {
  assert.equal(actionForHotkey('7'), null);
...
test('hotkeyForAction 反向映射到数字键', () => {
  ...
  assert.equal(hotkeyForAction('copy+pin'), '5');
  assert.equal(hotkeyForAction('ai'), '6');
});
...
test('数字动作映射单一来源且正反向往返对称', () => {
  ...
  assert.ok(source.includes("'5': 'copy+pin'"), '5 必须映射 copy+pin');
  assert.ok(source.includes("'6': 'ai'"), '6 必须映射 ai');
  ...
  for (const action of ['copy', 'save', 'pin', 'edit', 'copy+pin', 'ai']) {
...
test('动作映射全集封闭且每个动作恰好一个热键与防御式容错', () => {
  // 源码护栏：映射块必须恰好 6 个条目且与预期完全一致（加重复动作热键如 5: copy
  ...
  assert.deepEqual(entries, [['1', 'copy'], ['2', 'save'], ['3', 'pin'], ['4', 'edit'], ['5', 'copy+pin'], ['6', 'ai']], '映射条目必须与预期完全一致');
