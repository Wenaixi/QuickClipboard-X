...
test('actionForHotkey 数字键映射到对应动作', () => {
  ...
  assert.equal(actionForHotkey('4'), 'edit');
  assert.equal(actionForHotkey('5'), 'upload');
  assert.equal(actionForHotkey('6'), 'copy+pin');
  assert.equal(actionForHotkey('7'), 'ai');
});

test('actionForHotkey 未知键返回 null', () => {
  assert.equal(actionForHotkey('8'), null);
...
test('hotkeyForAction 反向映射到数字键', () => {
  ...
  assert.equal(hotkeyForAction('edit'), '4');
  assert.equal(hotkeyForAction('upload'), '5');
  assert.equal(hotkeyForAction('copy+pin'), '6');
  assert.equal(hotkeyForAction('ai'), '7');
});
...
test('数字动作映射单一来源且正反向往返对称', () => {
  ...
  assert.ok(source.includes("'4': 'edit'"), '4 必须映射 edit');
  assert.ok(source.includes("'5': 'upload'"), '5 必须映射 upload');
  assert.ok(source.includes("'6': 'copy+pin'"), '6 必须映射 copy+pin');
  assert.ok(source.includes("'7': 'ai'"), '7 必须映射 ai');
  ...
  for (const action of ['copy', 'save', 'pin', 'edit', 'upload', 'copy+pin', 'ai']) {
...
test('动作映射全集封闭且每个动作恰好一个热键与防御式容错', () => {
  // 源码护栏：映射块必须恰好 7 个条目且与预期完全一致（加重复动作热键
  ...
  assert.deepEqual(entries, [['1', 'copy'], ['2', 'save'], ['3', 'pin'], ['4', 'edit'], ['5', 'upload'], ['6', 'copy+pin'], ['7', 'ai']], '映射条目必须与预期完全一致');
