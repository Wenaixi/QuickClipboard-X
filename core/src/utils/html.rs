...
mod tests {
    ...
    /// capture.rs 与 paste/options.rs 待对应域迁入本 crate 时补回——
    /// 当前这些文件仍在 src-tauri 侧,core 内读不到。
    #[test]
    fn capture_and_paste_reuse_shared_is_image_only_html() {
        let root = env!("CARGO_MANIFEST_DIR");
        let options = fs::read_to_string(format!("{root}/src/services/paste/mod.rs"))
            .expect("读 paste/mod.rs");
