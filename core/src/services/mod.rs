...
mod tests {
    ...
    // 剥注释后再匹配,避免注释字面误命中。
    //
    // 跨域断言(data_management / commands / updater creator)随对应域迁入时补齐:
    // 那些文件尚在 src-tauri 侧,core 内读不到,本模块只保留对自身与 core 内
    // 调用点的断言。
    use crate::test_utils::{source_file, strip_line_comments};
    ...
    #[test]
    fn storage_is_portable_mode_delegates_to_helper() {
    ...
    fn sanitize_remote_files_content_leaves_other_content_untouched() {
        // 非 files: 内容原样返回;纯相对路径 files: 净化幂等——路径集合不变
        let text = "普通文本内容";
        ...
        let out = crate::services::sanitize_remote_files_content(clean);
        assert!(out.starts_with("files:"), "files: 前缀必须保留");
        let data: crate::services::paste::FilesData =
            serde_json::from_str(&out[6..]).expect("净化后仍必须是合法 files: JSON");
        let paths: Vec<&str> = data.files.iter().map(|f| f.path.as_str()).collect();
        assert_eq!(
            paths,
            vec!["clipboard_images/abc.png"],
            "纯相对路径 files: 内容必须原样保留(净化幂等)"
        );
    }
}
