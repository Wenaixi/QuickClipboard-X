...
mod tests {
    ...
    fn ai_key_failed_migration_does_not_trigger_save_which_would_erase_disk_plaintext() {
        ...
        let migrated_pos = source
            .find("|| had_legacy_ai_api_key && settings.ai_api_key.is_empty();")
            .expect("migrated 必须仅在成功清空后才计入 AI key 迁移");
        assert!(
            source[migrated_pos..].contains("if migrated {"),
            "save 触发必须严格依赖 migrated 成功态"
        );
        // 失败臂不修改明文:AI 迁移失败分支只 eprintln,不得 clear。
        // 用原文精确字面 `Err(e) => settings.ai_api_key.clear()` 做负向断言,
        // 且只匹配生产代码而非常量字符串——注释与断言剥除后原文只剩成功臂。
        let ok_clear = source.contains("Ok(()) => settings.ai_api_key.clear()");
        assert!(ok_clear, "成功臂必须清内存明文(迁移完成)");
        let failure_arms: Vec<_> = source.lines().filter(|l| {
            let l = l.trim_start();
            l.starts_with("Err(e) => settings.ai_api_key.clear()")
        }).collect();
        assert!(
            failure_arms.is_empty(),
            "失败臂禁止清内存明文(否则 migrated 判定与磁盘抹除双失守)"
        );
        assert!(
            source.contains("Err(e) => eprintln!(\"迁移 AI API key 到系统凭据库失败: {}\", e)"),
            "失败臂必须只 eprintln 保留明文(迁移失败时不抹除唯一明文副本)"
        );
    }
}
