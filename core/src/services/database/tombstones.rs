...
mod sync_tombstone_diff_guard {
    ...
    // LAN pull 墓碑差量护栏:fetch_peer_tombstones 必须携带本地 MAX(deleted_at)
    // 锚点(否则每次同步全量传输墓碑表);服务端 list_sync_tombstones_since
    // 必须以 deleted_at >= since 过滤——严格大于会在锚点等于对端墓碑
    // deleted_at 时确定性漏拉该墓碑且永不补拉,与 history/favorites 的
    // >= 增量口径一致。客户端接线必须用 lan_local_tombstones_max_deleted_at()
    // 取锚点。
    //
    // 说明:客户端侧两个断言(http_client.rs fetch_peer_tombstones / pull.rs
    // pull_from_peer)随 sync_transfer lan 域迁入本 crate 时补回——当前这些
    // 文件仍在 src-tauri 侧,core 内读不到。
    #[test]
