...
pub fn paste_merged_items(items: &[ClipboardItem]) -> Result<(), String> {
    let payload = build_merge_payload(items)?;
    ...
    // 粘贴后隐藏主窗口为 GUI 层职责（原 app: &tauri::AppHandle 隐藏逻辑），
    // core 只发出事件，窗口收起由 gui 在消费端处理。
    crate::events::post(crate::events::AppEvent::TrayMenuRefresh);
