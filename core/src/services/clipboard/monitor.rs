...
use crate::events::{emit_clipboard_updated, AppEvent, ClipboardUpdatedEvent};
use clipboard_rs::{
...
pub fn start_clipboard_monitor() -> Result<(), String> {
    ...
    // 启动剪贴板来源监控（start_clipboard_source_monitor 未迁入 core，
    // 来源信息改为 on-demand 读取，不启后台监视器线程）
...
pub fn stop_clipboard_monitor() -> Result<(), String> {
    ...
    if was_running {
        // 停止剪贴板来源监控（监视器线程未迁入 core，on-demand 读取无需停）
...
fn process_clipboard_change_once() {
                ...
                Ok(id) => {
                    ...
                    match crate::services::database::get_clipboard_item_by_id(id) {
                        Ok(Some(item)) => {
                            ...
                            let _ = emit_clipboard_updated(ClipboardUpdatedEvent {
    ...
    if any_stored {
        // LAN 变更通知（lan_notify_local_change）与音效自由函数
        // （copy_success_sound）尚在 src-tauri 侧，本轮迁入后补接；
        // 事件总线已统一承载剪贴板更新通知。
        crate::events::post(crate::events::AppEvent::ClipboardUpdated(
            crate::events::ClipboardUpdatedEvent {
                kind: "created".to_string(),
                item: None,
                insert_index: None,
                total_count: None,
        ...
        crate::services::AppSounds::play_copy_on_success();
    }
}
...
fn handle_clipboard_change() -> Result<(), String> {
    ...
    // 完整应用过滤（is_front_app_globally_disabled / is_current_app_allowed）
    // 随 app_filter 全量迁入时接入，本轮先保持放行。
    crate::services::AppSounds::play_copy_immediate();
