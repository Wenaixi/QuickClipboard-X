...
// 校验处再次枚举字面动作。copy+pin 是组合预设(复制后贴图),由引擎内联。
const SCREENSHOT_ACTIONS: [&str; 5] = ["copy", "save", "pin", "ai", "copy+pin"];
...
mod source_guards {
    ...
    fn action_validation_delegates_to_shared_action_constant() {
        ...
        //（否则动作链新增动作时校验与引擎两处漂移）。
        assert!(source.contains("const SCREENSHOT_ACTIONS: [&str; 5]"), "必须声明共享动作集常量");
